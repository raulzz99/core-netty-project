...
public class HeartbeatManager extends Thread {
	...
	public void setleaderStatus(String leaderNode, Integer leaderRank) {
		this.leaderNode = leaderNode;
		this.leaderRank = leaderRank;
		this.currNodeRank=leaderRank;
		this.isLeader=true;
		logger.info("setleaderStatus is "+this.isLeader);
	}
	
	/**
	 * Compares current leader's rank with its neighbouring nodes rank.
	 * Update current leader's rank if other has higher rank
	 * req : ack,change,none
	 */
	public void updateLeaderStatus(String neig_nodeID, Integer neig_rank,
			boolean alive) {
		logger.info("Inside  updateLeaderStatus 1 leader Node is  "
				+ neig_nodeID + "  rank is " + neig_rank);
		if (alive ) {
			if (leaderRank <= neig_rank) {
				// Do Nothing

			} else {
				logger.info("*********Leader Status getting updated. Leader Node switching from "+leaderNode+" with"
						+ " RANK "+leaderRank+" TO "+neig_nodeID +"  with RANKS "+neig_rank+" **********");
				leaderNode = neig_nodeID;
				leaderRank = neig_rank;// Update it new leader

			}
			logger.info("Inside  updateLeaderStatus 2 leader Node is  "
					+ leaderNode + "  rank is " + leaderRank);
		} else {
			
			leaderNode = nodeId;
			leaderRank =currNodeRank;
		}
		if(leaderNode.equals(nodeId)){
			isLeader=true;
		}
		else {
			isLeader=false;
	...
	private Management generateHB() {
		...
		h.setNodeId(leaderNode);
		h.setNodeRank(leaderRank);
	...
	public void run() {
		...
		while (forever) {
			try {
				...
				logger.info("************ Inside  HEartManager leader Node is  "
						+ leaderNode + "  rank is " + leaderRank+" ******************");
